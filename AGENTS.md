...
| Directory | Purpose |
|---|---|
| `tetris/game/` | Pure domain: `Board`, `Tetromino` (+ `SHAPES` rotation data), `PieceProvider` facade + `PieceGenerator` hierarchy (`RandomGenerator`, `BagGenerator`→`SevenBagGenerator`/`ThirtyFiveBagGenerator`, `WeightedGenerator`, `ReplayGenerator`), `ScoreEngine`, `GameStats`, `rules` (grid-agnostic game-rule functions + SRS kick data) |
...
- **Naming**: `PascalCase` classes, `snake_case` functions/variables, `UPPER_CASE` constants (in `settings.py`, `tetris/game/tetromino.py` for `SHAPES`, `tetris/game/rules.py` for `SRS_KICKS_*`).
...
- **Candidate generation**: soft-drop BFS (`soft_drop_placements` in `tetris/ai/candidates.py`) with SRS wall kicks (`SRS_KICKS_JLSTZ`, `SRS_KICKS_I`) — enumerates all reachable placements including overhangs. Placements are `Placement` NamedTuples `(piece_type, rot, px, py, hold)` — `shape` is derived from `SHAPES[piece_type][rot]`. Hold candidates: when `_can_hold` is True, the AI also enumerates placements for the held piece (or next piece if hold is empty), doubling the candidate space. Look-ahead depth configurable (`ai_lookahead_depth` 1–3): simulates best next-piece placement (Dellacherie-optimal) for N upcoming pieces. Hard-drop fallback when soft-drop is OFF (uses `iter_column_positions` helper shared by hard-drop and look-ahead paths). Shared game-rule functions are grid-agnostic in `tetris/game/rules.py`, used by both `Board` (list grid) and AI simulation (numpy grid).
