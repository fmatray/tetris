...
- **Imitation warm-start** (`ai_imitation`, default OFF): human games record each locked placement (piece, rotation, column, hold flag) to `human_placements.jsonl` via `PlacementsLog` (`tetris/game/imitation.py`) — recorder attached by `HumanState` only. When ON and learning mode, `imitation_pretrain(agent)` (`tetris/ai/imitation.py`) runs at `AIState.__init__` after `load()`: replays each recorded game on a numpy board, re-enumerates candidates with the same `get_candidate_states` used live, and applies a softmax cross-entropy ranking loss pushing `V(human placement)` up. Un-replayable moves skipped; missing log = no-op; failures never crash startup.
