...
def test_hyperparam_options_count():
    assert len(HyperparamMenuState._OPTIONS) == 20


def test_hyperparam_toggle_indices():
    assert HyperparamMenuState._toggle_indices == frozenset({1, 2, 3, 5, 6, 7, 8, 9, 11, 12, 13, 15, 16, 17})


def test_hyperparam_value_labels():
    state = _make_hyperparam()
    assert state._value_label(1) == f"{state.menu.ai_epsilon_decay:.4f}"
    assert state._value_label(2) == f"{state.menu.ai_epsilon_end:.2f}"
    assert state._value_label(6) == f"{state.menu.ai_gamma:.3f}"
    assert state._value_label(7) == str(state.menu.ai_batch_size)
    assert state._value_label(8) == f"{state.menu.ai_buffer_size:,}"
    ...
    assert state._value_label(18) == ""  # Reset
    assert state._value_label(19) == ""  # Back
...
def test_menu_base_prev_enabled_skips_disabled():
    ...
    state.selection = 5  # Règles du jeu (shifted +1)
    new = state._prev_enabled(5)
    # Should skip index 4 (MCP disabled) and 3 (IA disabled)
    assert new == 2  # Humain
...
def test_menu_state_disabled_ia_when_human():
    menu = _make_menu()
    menu.player = "Humain"
    assert menu._is_disabled(3) is True  # IA


def test_menu_state_disabled_human_when_ai():
    menu = _make_menu()
    menu.player = "IA"
    assert menu._is_disabled(2) is True  # Humain
...
def test_menu_state_select_mcp():
...
def test_menu_select_start_mcp():
    """Selecting 'Démarrer' with MCP player creates MCPState."""
    from tetris.states.mcp import MCPState

    menu = _make_menu()
    menu.player = "MCP"
    menu.selection = 0  # Démarrer
    result = menu._on_select()
